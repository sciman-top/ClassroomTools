    ...
    def _summarize_wps_process_hints(
        ...
        predicate_specs = (
            (
                ...
                _PresentationWindowMixin._is_wps_slideshow_class,
            ),
            (
                "has_wps_presentation_signature",
                self._class_has_wps_presentation_signature,
                self._normalized_has_wps_presentation_signature,
                _PresentationWindowMixin._class_has_wps_presentation_signature,
            ),
            (
                "has_ms_presentation_signature",
                self._class_has_ms_presentation_signature,
                self._normalized_has_ms_presentation_signature,
                _PresentationWindowMixin._class_has_ms_presentation_signature,
            ),
            (
                "has_writer_signature",
                self._class_has_wps_writer_signature,
                self._normalized_has_wps_writer_signature,
                _PresentationWindowMixin._class_has_wps_writer_signature,
            ),
        )

        memoized_predicates: Dict[str, Callable[[str], bool]] = {}
        for flag_name, predicate, normalized_impl, base_impl in predicate_specs:
            uses_base = getattr(predicate, "__func__", None) is base_impl
            memoized_predicates[flag_name] = _memoize(
                predicate, normalized_impl if uses_base else None
            )

        flags = {name: False for name, *_ in predicate_specs}
